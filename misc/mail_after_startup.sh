...
if [ $CRASH -eq 0 ]; then
	SUBJECT="`echo "pfSense $ZONE: System startup: "` `date +%Y%m%d-%H%M`"
else
	SUBJECT="`echo "pfSense $ZONE: System startup after crash: "` `date +%Y%m%d-%H%M`"	
fi
BODY=`echo "(mail generated by script pfSense:///home/marsPortal/misc/mail_after_startup.sh)"`
