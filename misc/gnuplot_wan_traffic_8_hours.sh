...
select 
    (((max(tx)-min(tx)) / (select count(*) from log_wan_traffic where when2 >= now() - INTERVAL 8 HOUR)) * -1) as tx_avg,
    ((max(rx)-min(rx)) / (select count(*) from log_wan_traffic where when2 >= now() - INTERVAL 8 HOUR)) as rx_avg
from log_wan_traffic where when2 >= now() - INTERVAL 8 HOUR;
EOF
)
...
select min(when2), max(rx)-min(rx), sum(rx) from log_wan_traffic where when2 >= date_sub(now(), interval 1 hour);
