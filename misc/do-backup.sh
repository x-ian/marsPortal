...
CONFIG_FILE=/tmp/config-router-$DATE.xml
# login
...
/usr/bin/tar czf $FILE -C /tmp *$DATE*
...
$BASEDIR/misc/send_mail_with_attachment.sh "$SUBJECT" "$BODY `cat $TMP_LOGFILE`" $FILE $FILE application/x-compressed
