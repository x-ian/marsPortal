...
# login
/usr/local/bin/wget -qO/dev/null --keep-session-cookies --save-cookies $TMP_COOKIES \
 --post-data "login=Login&usernamefld=`echo $USER`&passwordfld=`echo $PASSWD`" \
 --no-check-certificate $PF_SERVER/diag_backup.php

# get all active users
/usr/local/bin/wget --keep-session-cookies --load-cookies $TMP_COOKIES --no-check-certificate --output-document=$TMP_ALL \
...
do
  /usr/local/bin/wget --keep-session-cookies --load-cookies $TMP_COOKIES --no-check-certificate  --output-document=$TMP_ALL2 \
