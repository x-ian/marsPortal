...
<? 
$result = mysqli_query("SELECT u.username AS username, u.firstname AS firstname, u.lastname AS lastname, u.mac_vendor AS mac_vendor, u.hostname AS hostname, u.email AS email, u.department AS department, u.organisation AS organisation, u.registration_date AS registration_date, u.initial_ip AS initial_ip, u.notes AS notes, radusergroup.groupname AS groupname FROM userinfo AS u LEFT JOIN radusergroup on u.username = radusergroup.username ORDER BY registration_date DESC") or trigger_error(mysqli_error()); 
...
<td>
	...
	<a class="btn btn-default btn-xs" href=/mars/reports/device-traffic-details.php?username=<?=$row['username']?>>Traffic&nbsp;Details</a>&nbsp;<a class="btn btn-default btn-xs" href=/mars/reports/mars/device_with_volume.php?username=<?=$row['username']?>>Traffic&nbsp;History</a>&nbsp;<a class="btn btn-default btn-xs" href=/mars/reports/device-activity.php?username=<?=$row['username']?>>Activity&nbsp;History</a>&nbsp;
