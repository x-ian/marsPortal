...
if [[ $WORK_DAYS == *"$NOW_DAY"* ]] && [ "$WORK_START" -le "$NOW" ] && [ "$WORK_END" -ge "$NOW" ] && [ "$WORK_OFFSET_ALREADY_SET" -eq "0" ]; then
	update_accounting "work_offset" "work_offset_input" "work_offset_output"
fi
if [[ $WORK_DAYS == *"$NOW_DAY"* ]] && [ "$LUNCH_START" -le "$NOW" ] && [ "$LUNCH_END" -ge "$NOW" ] && [ "$LUNCH_OFFSET_ALREADY_SET" -eq "0" ]; then
