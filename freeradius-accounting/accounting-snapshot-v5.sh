...
INSERT INTO daily_accounting_v5 (username, day, offset_input, offset_output, offset)
  ...
  WHERE ra.username not in 
    (select ra.username from daily_accounting_v5 where day = date_format(now(), '%Y-%m-%d'))
EOF
...
SET `echo $NEXT_INTERVAL_COLUMN`_input = (ra.input_octets - offset_input), `echo $NEXT_INTERVAL_COLUMN`_output = (ra.output_octets - offset_output)
WHERE da.day = date_format(now(), '%Y-%m-%d');
EOF
}

# current time and day in week
CLOSEST_HALF_HOUR_TMP=`echo "$(date "+%s") - ($(date +%s)%1800)" | /usr/bin/bc`
CLOSEST_HALF_HOUR_TMP2=`echo "$CLOSEST_HALF_HOUR_TMP + 1800 - 60" | /usr/bin/bc`
